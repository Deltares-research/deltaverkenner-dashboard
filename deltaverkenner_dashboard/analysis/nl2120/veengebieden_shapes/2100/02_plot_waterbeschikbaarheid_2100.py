...
for watervraag_type in watervraag_types:
    ...
    for selected_month in selected_months:
        ...
        data["Watervraag gecorrigeerd"] = (1 - 0.27) * data["Watervraag"]

        result_veengebieden = data.merge(mapping, on="deelregio")

        result_veengebieden["watervraag_deelgebied"] = (
            result_veengebieden["Watervraag gecorrigeerd"] * result_veengebieden["pct"]
        )
        ...
        ).agg({"watervraag_deelgebied": "sum", "geometry": "first", "id_num": "first"})

        deelgebied_gdf = gpd.GeoDataFrame(
            deelgebied_gdf, geometry="geometry", crs=veengebieden_with_watervraag.crs
        )
        ...
        ax.set_title("Waterbeschikbaarheid in 2100", fontsize=14, loc="right")
        ...
        figpath = Path(
            f"p:/11212687-deltaverkenner2026/Zoetwater/Dashboard/data/nl2120/figuren/shapes_veengebieden/2100/Figuren Dimmie/02_waterbeschikbaarheid_2100_deelregios_{selected_month}_1976.png"
        )
        ...
        deelgebied_gdf = deelgebied_gdf.drop(columns=["geometry", "id_num"])

        deelgebied_gdf = deelgebied_gdf.rename(
            columns={"watervraag_deelgebied": "Waterbeschikbaarheid (m3/s)"}
        )

        outputpath = f"p:/11212687-deltaverkenner2026/Zoetwater/Dashboard/data/nl2120/figuren/shapes_veengebieden/2100/csv's Dimmie/02_waterbeschikbaarheid_2100_deelgebieden_{selected_month}_1976.csv"

        deelgebied_gdf.to_csv(outputpath, index=False)
